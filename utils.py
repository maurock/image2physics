# File created in https://github.com/ajzhai/NeRF2Physics/blob/master/utils.py, MIT License
import os
import numpy as np
import json
...
def load_images(img_dir, bg_change=255, return_masks=False):
    ...
    for img_file in img_files:
        ...
        if img.shape[2] == 3:
            img = np.concatenate([img, 255 * np.zeros((img.shape[0], img.shape[1], 1), dtype=np.uint8)], axis=2)
...
def load_depths(depth_dir, Ks):
    ...
    depth_files = sorted(depth_files, key=lambda x: int(x[3:x.index('.npy')])) 
    depths = []
